package com.example.yifanyang.detectiontarget;
        ...
        if(src1Selected && src2Selected){
            ...
            new AsyncTask<Void, Void, Bitmap>() {
                ...
                protected void onPostExecute(Bitmap bitmap) {
                    ...
                    tvKeyPointsObject1.setText("目标1："+keypointsObject1);
                    tvKeyPointsObject2.setText("目标2："+keypointsObject2);
                    tvKeyPointsMatches.setText("关键点匹配："+keypointMatches);
                    tvTime.setText("耗费时间："+(endTime-startTime)+" ms");
                }
            }.execute();
        }
    }
...
//                descriptorMatcher = DescriptorMatcher.create(DescriptorMatcher.FLANNBASED);
        ...
        Log.i("APP", CvType.typeToString(src1.type())+" "+CvType.typeToString(src2.type()));
...
//        Features2d.drawMatches(src1, keypoints1, src2, keypoints2, matches, src3);
    ...
    static Mat drawMatches(Mat img1, MatOfKeyPoint key1, Mat img2, MatOfKeyPoint key2, MatOfDMatch matches, boolean imageOnly){
        ...
        Imgproc.putText(out, "帧", new Point(img1.width() / 2,30), Core.FONT_HERSHEY_PLAIN, 2, new Scalar(0,255,255),3);
        Imgproc.putText(out, "已匹配", new Point(img1.width() + img2.width() / 2,30), Core.FONT_HERSHEY_PLAIN, 2, new Scalar(255,0,0),3);
        return out;
    }
